...
import { Mail, Phone, Building, Calendar, User, MapPin, AlertCircle, FileText } from 'lucide-react';

// Define UserProfile interface to match API response
interface UserProfile {
  ...
  status?: number | null;
  earningAssociations?: Array<{
    id: number;
    employeeId: number;
    tenantEarningId: number;
    calculationValue: number;
    tenantearning: any | null;
  }>;
  accessableFeatures?: Array<{
    id: string;
    name: string;
    key: string;
  }>;
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        console.log('Setting profile:', response);
        setProfile(response);
      } catch (error: any) {
        ...
        if (retryCount < maxRetries && error.response?.status >= 500) {
          ...
          setTimeout(() => fetchProfile(), 1000 * (retryCount + 1));
          return;
        }
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: error.response?.data?.message || "Failed to load profile information. Please try again later.",
        });
      } finally {
        setIsLoading(false);
      }
                  ...
                  <CardContent className="relative pt-0 px-8 pb-8">
                    <div className="flex flex-col md:flex-row items-start md:items-end -mt-20 mb-8 gap-8">
                      <Avatar className="w-36 h-36 border-4 border-white shadow-md">
                        <AvatarImage src={formatValue(profile.imageUrl)} alt={`${profile.firstName} ${profile.lastName}`} />
                  ...
                  <CardContent className="pt-6 px-8">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-y-8 gap-x-6">
                      ...
                      <InfoItem
                        ...
                        value={`${formatValue(profile.experience?.toString())} years (${formatValue(profile.totalExperience?.toString())} total)`}
                      />
                      <InfoItem
                        icon={<AlertCircle size={20} />}
